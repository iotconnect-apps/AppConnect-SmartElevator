...
import { Notification, NotificationService, DeviceService, ScheduledMaintenanceService, ElevatorService } from 'app/services';
...
export class DynamicDashboardComponent implements OnInit, OnDestroy {
  ...
  countData: any = {
    ...
    totalElevators: '-',
    elevators: [],
    totalConnectedElevators: '-',
    totalDisconnectedElevators: '-',
    totalEnergyCount: '-',
  ...
  getDashbourdCount() {
    ...
    this.dashboardService.getDashboardoverview().subscribe(response => {
      if (response.isSuccess === true) {
        this.countData.totalAlert = (response.data.totalAlert) ? response.data.totalAlert : 0
        this.countData.totalConnectedElevators = (response.data.totalConnectedElevators) ? response.data.totalConnectedElevators : 0;
        this.countData.totalElevators = (response.data.totalElevators) ? response.data.totalElevators : 0;;
        this.countData.totalBuilding = (response.data.totalBuilding) ? response.data.totalBuilding : 0;
        this.countData.totalEnergyCount = (response.data.totalEnergyCount) ? response.data.totalEnergyCount : 0;
        this.countData.elevators.push({ name: (response.data.minElevatorName ? response.data.minElevatorName : ''), consumption: response.data.minElevatorEnergyCount });
        this.countData.elevators.push({ name: (response.data.maxElevatorName ? response.data.maxElevatorName : ''), consumption: response.data.maxElevatorEnergyCount });
        this.countData.totalUnderMaintenanceCount = (response.data.totalUnderMaintenanceCount) ? response.data.totalUnderMaintenanceCount : 0
        this.countData.totalScheduledCount = (response.data.totalScheduledCount) ? response.data.totalScheduledCount : 0
    ...
    if (is_save == 'y') {
      if (this.dashboardWidgets[this.editWidgetIndex].componentName == 'widget-chart-b') {
        if (this.dashboardWidgets[this.editWidgetIndex].widgetProperty.telemetryUniqueId != '') {
          ...
          this._notificationService.handleResponse({ message: "You must select at least one elevator to save widget." }, "error");
          isProcess = false;
        }
      }
    }
      ...
      for (var i = this.dashboardWidgets.length - 1; i >= 0; i--) {
        if (this.dashboardWidgets[i].componentName == 'widget-chart-b') {
          if (this.dashboardWidgets[i].widgetProperty.telemetryUniqueId == '') {
            this._notificationService.handleResponse({ message: "You must select at least one generator for telemetry to save widget." }, "error");
            isOk = false;
          }
        }
      }
  ...
  deviceChange(widgetIndex: number = 0, isClear: string = 'n') {
    ...
    this.getElevatorAttributesData(widgetIndex);
  ...
  telemetryDeviceChange(index: number = 0) {
    ...
    this.getElevatorAttributesData(index);
    this.telemetryDeviceChangeEvent.emit(this.dashboardWidgets[index]);
    this.changeDetector.detectChanges();
  }

  getElevatorAttributesData(widgetIndex) {
    if (this.dashboardWidgets[widgetIndex].widgetProperty.telemetryUniqueId != '') {
      ...
      if (device && device.length > 0 && device[0].guid) {
        this.elevatorService.getelevatorDetails(device[0].guid).subscribe(response => {
          if (response.isSuccess === true && response.data && response.data.templateGuid && response.data.templateGuid != '') {
            this.deviceData[deviceIndex].templateGuid = response.data.templateGuid;
            device[0].templateGuid = response.data.templateGuid;
          }
          this.spinner.show();
          this.elevatorService.getelevatorTelemetryData(device[0].templateGuid).subscribe(response => {
            this.spinner.hide();
            if (response.isSuccess === true) {
