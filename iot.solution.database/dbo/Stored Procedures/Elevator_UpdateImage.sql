...
001	sgh-1 05-12-2019 [Nishit Khakhi]	Added Initial Version to Update Elevator Image
*******************************************************************/

CREATE PROCEDURE [dbo].[Elevator_UpdateImage]
(
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param =
        (
            SELECT 'Elevator_UpdateImage' AS '@procName'
	...
	BEGIN TRY
		IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[Elevator] (NOLOCK) WHERE [guid] = @guid AND [companyGuid]=@companyGuid AND [isDeleted]=0)
		BEGIN
			Set @output = -3
