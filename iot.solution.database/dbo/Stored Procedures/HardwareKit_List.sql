...
003	SSES-2 11-03-2020 [Nishit Khakhi]	Updated tag to replace with tagGuid
*******************************************************************/
...
BEGIN
    ...
    BEGIN TRY            
		...
		SET @Sql = ' SELECT *,ROW_NUMBER() OVER (ORDER BY '+@orderby+') AS row_num
			...
			LEFT JOIN [dbo].[Elevator] D  WITH (NOLOCK) ON h.[uniqueId] = D.[uniqueId] ANd D.[isDeleted] = 0
